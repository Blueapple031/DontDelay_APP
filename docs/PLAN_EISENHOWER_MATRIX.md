...
- 할 일 화면에서 **보기 전환**: 기존 **칸반**(해야 할 일 / 진행 중 / 완료) ↔ **아이젠하워 플롯**.
- 아이젠하워 모드에서 **가로 = 긴급도(0~10)**, **세로 = 중요도(0~10)** 인 2차원 영역에 카드를 배치한다.
...
- 헤더 영역에 **토글**(예: `SegmentedButton` 또는 탭): `칸반` | `아이젠하워`.
...
3. **토글** — `TodoScreen`에서 칸반 / 아이젠하워 분기.
4. **아이젠하워 위젯** — 축·분면 배경·플롯 영역 `LayoutBuilder`로 크기 확보.
...
|------|------|
| 2026-05-28 | 초안 작성 |
| 2026-05-27 | 칸반/아이젠하워 토글·`TodoItem` `urgency`/`importance`·드롭 매핑·JSON 반영 구현 |
