...
class _TodoDialogState extends ConsumerState<_TodoDialog> {
  ...
  Widget build(BuildContext context) {
    final tags = ref.watch(tagListProvider).value ?? [TagItem.defaultTag];
...
class _TagEditDialogState extends ConsumerState<_TagEditDialog> {
  ...
  void _startEdit(TagItem tag) {
    _nameCtls.putIfAbsent(
        tag.id, () => TextEditingController());
  ...
  Future<void> _saveEdit(TagItem tag) async {
    final name =
        _nameCtls[tag.id]?.text.trim() ?? tag.name;
  ...
  Future<void> _addNew() async {
    ...
    if (name.isEmpty) return;
    try {
      await ref.read(tagListProvider.notifier).addTag(
            TagItem(name: name, colorHex: _newColor),
          );
      _newNameCtl.clear();
      setState(() {
        _addingNew = false;
  ...
  Widget build(BuildContext context) {
    final tags =
        ref.watch(tagListProvider).value ?? [TagItem.defaultTag];
