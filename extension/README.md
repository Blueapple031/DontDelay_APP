...
3. 저장 방법 (택 1):
   - popup **현재 탭 저장** (메모 선택 가능)
   - 페이지/링크 **우클릭 → DontDelay에 저장** → 메모 선택 팝업
   - 단축키 **Alt+Shift+S** (메모 없이 즉시 저장)
...
- manifest에 `http://127.0.0.1:*/*` 는 **사용할 수 없습니다** (포트 `:*` 와일드카드 거부).
- `optional_host_permissions` + 사용 시 권한 허용 팝업으로 처리합니다.
- ZIP 업로드 전 `extension` 폴더를 압축하세요 (상위 `DontDelay` 폴더 전체가 아님).

## 문제 해결

| 증상 | 해결 |
|------|------|
| 스토어: 잘못된 url `127.0.0.1:*` | manifest 1.0.4+ 사용, 확장 새로고침 후 ZIP 재생성 |
